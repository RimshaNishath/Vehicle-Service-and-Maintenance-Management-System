...
if (isset($_POST['update_customer'])) {
    ...
    if (!preg_match('/^\d{7,15}$/', $phone)) {
        ...
        $stmt = mysqli_prepare($conn,
            "UPDATE Customer SET Name=?, Phone_Number=?, Address=? WHERE Customer_ID=?");
...
if (isset($_POST['update_vehicle'])) {
    ...
    if (empty($vehicle_number) || empty($vehicle_type) || $customer_id <= 0) {
        ...
        $stmt = mysqli_prepare($conn,
            "UPDATE Vehicle SET Customer_ID=?, Vehicle_Number=?, Vehicle_Type=? WHERE Vehicle_ID=?");
        mysqli_stmt_bind_param($stmt, "issi", $customer_id, $vehicle_number, $vehicle_type, $id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: view.php");
            exit;
        } else {
...
if (isset($_POST['update_service'])) {
    ...
    if (empty($service_type) || empty($service_date) || $vehicle_id <= 0) {
        ...
        $stmt = mysqli_prepare($conn,
            "UPDATE Service SET Vehicle_ID=?, Service_Type=?, Service_Date=? WHERE Service_ID=?");
        mysqli_stmt_bind_param($stmt, "issi", $vehicle_id, $service_type, $service_date, $id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: view.php");
            exit;
        } else {
...
$vehicles  = mysqli_query($conn,
    "SELECT v.Vehicle_ID, v.Vehicle_Number, c.Name
...
<head>
    ...
    <title>Update Record – VehicleServ</title>
...
<div class="container">
    <div class="page-header">
        <h1>Update Record</h1>
        <a href="view.php" class="btn btn-secondary btn-sm">&larr; Back</a>
    </div>
        ...
        <form method="POST" action="update.php">
            ...
            <div class="form-group">
                ...
                <input type="text" name="phone"
                       value="<?= htmlspecialchars($record['Phone_Number']) ?>"
                       pattern="\d{7,15}" title="7–15 digits only" required>
            </div>
            <div class="form-group">
                <label>Address</label>
                <input type="text" name="address"
                       value="<?= htmlspecialchars($record['Address']) ?>" required>
            </div>
        ...
        <form method="POST" action="update.php">
                ...
                <select name="customer_id" required>
                    <?php while ($c = mysqli_fetch_assoc($customers)): ?>
                        <option value="<?= $c['Customer_ID'] ?>"
                            <?= $c['Customer_ID'] == $record['Customer_ID'] ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <input type="text" name="vehicle_number"
                       value="<?= htmlspecialchars($record['Vehicle_Number']) ?>" required>
            </div>
            <div class="form-group">
                <label>Vehicle Type</label>
                <select name="vehicle_type" required>
                    <?php foreach (['Sedan','SUV','Truck','Van','Motorcycle','Bus','Other'] as $vt): ?>
                        <option <?= $vt === $record['Vehicle_Type'] ? 'selected' : '' ?>>
                            <?= $vt ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <form method="POST" action="update.php">
                ...
                <select name="vehicle_id" required>
                    <?php while ($v = mysqli_fetch_assoc($vehicles)): ?>
                        <option value="<?= $v['Vehicle_ID'] ?>"
                            <?= $v['Vehicle_ID'] == $record['Vehicle_ID'] ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <select name="service_type" required>
                    ...
                    ?>
                        <option <?= $st === $record['Service_Type'] ? 'selected' : '' ?>>
                            <?= $st ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="date" name="service_date"
                       value="<?= htmlspecialchars($record['Service_Date']) ?>" required>
            </div>
